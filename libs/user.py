class Users:
    ...
    def current_status(self):
        if self.active:
            print(
                "Current status of user %d is active:\nRB_size: %d\nRB_num_req: %d\nRB_num_ass: %d\nRB_start is %d\n"
                % (self.user_info['id'], self.rb_size, self.rb_num_req,
                   self.rb_num_ass, self.rb_start))
        else:
            print(
                "Current status of user %d is inactive:\nRB_size: %d\nRB_num_req: %d\n"
                % (self.user_info['id'], self.rb_size, self.rb_num_req))
...
class eMBB_User(Users):
    ...
    rate_slot: a float indicating the local average data rate in the current time slot.  
    ...
    def current_status(self):
        if self.active:
            ...
            print("Current eMBB user status of eMBB user %d is inactive\n" %
                  (self.user_info['id']))
        print(
            "Current RB status of user %d is:\nRB_size: %d\nRB_num_req: %d\nRB_num_ass: %d\nRB_start is %d\n"
            % (self.user_info['id'], self.rb_size, self.rb_num_req,
               self.rb_num_ass, self.rb_start))
        print(
            "Current Rate status of user %d is:\nDRC: %f\nslot length: %d\nrate in this slot: %f\naverage rate is %f\n"
            % (self.user_info['id'], self.DRC, self.slot_len, self.rate_slot,
               self.rate_avg))
...
class URLLC_User(Users):
    ...
    Attributes:
        ...
        active: a boolean indicating the active status of the user.
    """
    def __init__(self, uid, rb_size, rb_req, slot_len, slot_start, retrans,
    ...
    def current_status(self):
        ...
        print(
            ...
            % (self.user_info['id'], self.rb_size, self.rb_num_req,
               self.rb_num_ass, self.rb_start))
        print(
            "Current status of URLLC user %d is:\nLatency constraint: %d ms\nmini slot length: %d\nstart in slot: %d\nretransmission times: %d\n"
            % (self.user_info['id'], self.latency, self.slot_len,
               self.slot_start, self.retrans))
