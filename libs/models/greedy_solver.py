            ...
            for k in range(rb_num_avi - num_req):
                ...
                for j in range(num_req):
                    if self.RB_map.bitmap[rb_start+j] > 0:
                        embb_user = embb_user_lists[self.RB_map.bitmap[rb_start+j]-1]
                        if embb_user.active == 0 or int(embb_user.user_info['id']) != self.RB_map.bitmap[rb_start+j]:
                            print ("ERROR: embb user mismatched!")
                        else:
                            embb_user.replace_num += 1
                            #urllc_user.ori_embb.append(self.RB_map.bitmap[rb_start+j])
                            #self.RB_map.bitmap[rb_start+j] = int(urllc_user.user_info['id'])
                #update throughput
