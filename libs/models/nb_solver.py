...
class CccpURLLCSolver(NaiveURLLCSolver):
    ...
    def allocate_resource(self):
        ...
        current_rate = self.engine.dc_main(matlab.double(embb_num), matlab.double(total_RB), matlab.double(y_start_end), matlab.double(embb_rate), matlab.double(embb_rb_ass), matlab.double(urllc_num), matlab.double(urllc_rb_perusr), matlab.double(drc))
        c = []
        for _ in range(current_rate.size[1]):
            c.append(current_rate._data[_*current_rate.size[0]:_*current_rate.size[0]+current_rate.size[0]].tolist())
