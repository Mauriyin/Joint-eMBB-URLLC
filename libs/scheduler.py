        ...
        for i in self.user_indexes:
            ...
            if rb_current >= embb_user.rb_num_req:
                ...
                embb_user.rate_avg = (
                    ...
                    embb_user.DRC) / (embb_user.sche_times + 1)
                embb_user.rate_cur = embb_user.DRC
                embb_user.sche_times += 1
                embb_user.rb_start = rb_start
                self.RB_map.bitmap[rb_start:embb_user.rb_num_req +
                                   rb_start] = int(embb_user.user_info['id'])
                rb_start += embb_user.rb_num_ass
                rb_current -= embb_user.rb_num_req
            else:
                if rb_current > 0:
                    embb_user.rb_num_ass = rb_current
                    new_DRC = (rb_current * embb_user.rb_size /
                               1000) / embb_user.slot_len
                    embb_user.rate_avg = (
                        embb_user.rate_avg * embb_user.sche_times +
                        new_DRC) / (embb_user.sche_times + 1)
                    embb_user.rate_cur = new_DRC
                    embb_user.sche_times += 1
                    embb_user.rb_start = rb_start
                    self.RB_map.bitmap[rb_start:rb_current + rb_start] = int(
                        embb_user.user_info['id'])
                    rb_current = 0
                else:
                    embb_user.rate_avg = (embb_user.rate_avg *
                                          embb_user.sche_times) / (
                                              embb_user.sche_times + 1)
                    embb_user.rb_start = -1
                    embb_user.rb_num_ass = 0
    ...
    def _user_sort(self):
        ...
        self.user_indexes = [
            index for index, value in sorted(
                list(enumerate(pf_weight)), key=lambda x: x[1], reverse=True)
        ]
