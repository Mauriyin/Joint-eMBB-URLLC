...
class RB:
    ...
    Attributes:
        rb_avi: an int indicating the total number of available rb units. 
        rb_size: an int indicating the size of one rb unit.
        bitmap: an int array, storing the assign status for each rb, 
        i indicates rb assigned to User i (i > 0), inited to 0.
    """
    ...
    def find_all_avi_rb(self, rb_num_req):
        ...
        while (rb_start < len(self.bitmap)):
            rb_start = get_safe_true_start(
                (self.bitmap[rb_valid_end:] <= self.border_bitmap_id)
                & (self.bitmap[rb_valid_end:] >= 0))
            if rb_start is None:
                return rb_start_list, num_ass_list
            else:
                rb_start = rb_start + rb_valid_end
            rb_valid_end = get_safe_true_start(
                self.bitmap[rb_start:] > self.border_bitmap_id)
